package com.cfk.xiaov.ui.adapter;
...
import com.cfk.xiaov.util.VideoThumbLoader;
import com.cfk.xiaov.widget.BubbleImageView;
import com.cfk.xiaov.widget.CircularProgressBar;
import com.lqr.adapter.LQRAdapterForRecyclerView;
import com.lqr.adapter.LQRViewHolderForRecyclerView;
import com.lqr.emoji.MoonUtils;
import com.cfk.xiaov.ui.presenter.SessionAtPresenter;
...
public class SessionAdapter extends LQRAdapterForRecyclerView<Message> {
    ...
    private static final int SEND_TEXT = com.cfk.xiaov.R.layout.item_text_send;
    private static final int RECEIVE_TEXT = com.cfk.xiaov.R.layout.item_text_receive;
    private static final int SEND_IMAGE = com.cfk.xiaov.R.layout.item_image_send;
    private static final int RECEIVE_IMAGE = com.cfk.xiaov.R.layout.item_image_receive;
    private static final int SEND_STICKER = com.cfk.xiaov.R.layout.item_sticker_send;
    private static final int RECEIVE_STICKER = com.cfk.xiaov.R.layout.item_sticker_receive;
    private static final int SEND_VIDEO = com.cfk.xiaov.R.layout.item_video_send;
    private static final int RECEIVE_VIDEO = com.cfk.xiaov.R.layout.item_video_receive;
    private static final int SEND_LOCATION = com.cfk.xiaov.R.layout.item_location_send;
    private static final int RECEIVE_LOCATION = com.cfk.xiaov.R.layout.item_location_receive;
    private static final int RECEIVE_NOTIFICATION = com.cfk.xiaov.R.layout.item_notification;
    private static final int RECEIVE_VOICE = com.cfk.xiaov.R.layout.item_audio_receive;
    private static final int SEND_VOICE = com.cfk.xiaov.R.layout.item_audio_send;
    private static final int RECEIVE_RED_PACKET = com.cfk.xiaov.R.layout.item_red_packet_receive;
    private static final int SEND_RED_PACKET = com.cfk.xiaov.R.layout.item_red_packet_send;
    private static final int UNDEFINE_MSG = com.cfk.xiaov.R.layout.item_no_support_msg_type;
    private static final int RECALL_NOTIFICATION = com.cfk.xiaov.R.layout.item_notification;
    ...
    private void setView(LQRViewHolderForRecyclerView helper, Message item, int position) {
        ...
        if (msgContent instanceof TextMessage) {
            MoonUtils.identifyFaceExpression(mContext, helper.getView(com.cfk.xiaov.R.id.tvText), ((TextMessage) msgContent).getContent(), ImageSpan.ALIGN_BOTTOM);
        } else if (msgContent instanceof ImageMessage) {
            ImageMessage imageMessage = (ImageMessage) msgContent;
            BubbleImageView bivPic = helper.getView(com.cfk.xiaov.R.id.bivPic);
            Glide.with(mContext).load(imageMessage.getLocalUri() == null ? imageMessage.getRemoteUri() : imageMessage.getLocalUri()).error(com.cfk.xiaov.R.mipmap.default_img_failed).override(UIUtils.dip2Px(80), UIUtils.dip2Px(150)).centerCrop().into(bivPic);
            ...
            if (MediaFileUtils.isImageFileType(fileMessage.getName())) {
                ImageView ivPic = helper.getView(com.cfk.xiaov.R.id.ivSticker);
                Glide.with(mContext).load(fileMessage.getLocalPath() == null ? fileMessage.getMediaUrl() :
                        fileMessage.getLocalPath()).placeholder(com.cfk.xiaov.R.mipmap.default_img).error(com.cfk.xiaov.R.mipmap.default_img_failed).centerCrop().into(ivPic);
                ...
                if (fileMessage.getLocalPath() != null && new File(fileMessage.getLocalPath().getPath()).exists()) {
                    ...
                    bivPic.setImageResource(com.cfk.xiaov.R.mipmap.img_video_default);
                }
            }
        } else if (msgContent instanceof LocationMessage) {
            LocationMessage locationMessage = (LocationMessage) msgContent;
            helper.setText(com.cfk.xiaov.R.id.tvTitle, locationMessage.getPoi());
            ImageView ivLocation = helper.getView(com.cfk.xiaov.R.id.ivLocation);
            Glide.with(mContext).load(locationMessage.getImgUri()).placeholder(com.cfk.xiaov.R.mipmap.default_location).centerCrop().into(ivLocation);
            ...
            try {
                ...
                helper.setText(com.cfk.xiaov.R.id.tvNotification, notification);
            ...
            RelativeLayout rlAudio = helper.setText(com.cfk.xiaov.R.id.tvDuration, voiceMessage.getDuration() + "''").getView(com.cfk.xiaov.R.id.rlAudio);
            ...
            helper.setText(com.cfk.xiaov.R.id.tvRedPacketGreeting, redPacketMessage.getContent());
            ...
            if (operatorId.equalsIgnoreCase(UserCache.getId())) {
                operatorName = UIUtils.getString(com.cfk.xiaov.R.string.you);
            } else {
                if (mPresenter.mConversationType == Conversation.ConversationType.PRIVATE) {
                    operatorName = UIUtils.getString(com.cfk.xiaov.R.string.other_party);
                } else {
            ...
            helper.setText(com.cfk.xiaov.R.id.tvNotification, UIUtils.getString(com.cfk.xiaov.R.string.recall_one_message, operatorName));
        }
    }

    private void setOnClick(LQRViewHolderForRecyclerView helper, Message item, int position) {
        helper.getView(com.cfk.xiaov.R.id.llError).setOnClickListener(v ->
        ...
        helper.getView(com.cfk.xiaov.R.id.ivAvatar).setOnClickListener(v -> {
    ...
    private void setStatus(LQRViewHolderForRecyclerView helper, Message item, int position) {
        ...
        if (msgContent instanceof TextMessage || msgContent instanceof LocationMessage || msgContent instanceof VoiceMessage) {
            ...
            if (sentStatus == Message.SentStatus.SENDING) {
                helper.setViewVisibility(com.cfk.xiaov.R.id.pbSending, View.VISIBLE).setViewVisibility(com.cfk.xiaov.R.id.llError, View.GONE);
            } else if (sentStatus == Message.SentStatus.FAILED) {
                helper.setViewVisibility(com.cfk.xiaov.R.id.pbSending, View.GONE).setViewVisibility(com.cfk.xiaov.R.id.llError, View.VISIBLE);
            } else if (sentStatus == Message.SentStatus.SENT) {
                helper.setViewVisibility(com.cfk.xiaov.R.id.pbSending, View.GONE).setViewVisibility(com.cfk.xiaov.R.id.llError, View.GONE);
            }
        } else if (msgContent instanceof ImageMessage) {
            ImageMessage imageMessage = (ImageMessage) msgContent;
            BubbleImageView bivPic = helper.getView(com.cfk.xiaov.R.id.bivPic);
            ...
            if (isSend) {
                ...
                if (sentStatus == Message.SentStatus.SENDING) {
                    ...
                    helper.setViewVisibility(com.cfk.xiaov.R.id.llError, View.GONE);
                } else if (sentStatus == Message.SentStatus.FAILED) {
                    bivPic.setProgressVisible(false);
                    bivPic.showShadow(false);
                    helper.setViewVisibility(com.cfk.xiaov.R.id.llError, View.VISIBLE);
                } else if (sentStatus == Message.SentStatus.SENT) {
                    bivPic.setProgressVisible(false);
                    bivPic.showShadow(false);
                    helper.setViewVisibility(com.cfk.xiaov.R.id.llError, View.GONE);
            ...
            if (MediaFileUtils.isImageFileType(fileMessage.getName())) {
                ...
                CircularProgressBar cpbLoading = helper.getView(com.cfk.xiaov.R.id.cpbLoading);
                if (isSend) {
                    Message.SentStatus sentStatus = item.getSentStatus();
                    if (sentStatus == Message.SentStatus.SENDING || fileMessage.getLocalPath() == null || (fileMessage.getLocalPath() != null && !new File(fileMessage.getLocalPath().getPath()).exists())) {
                        ...
                        helper.setViewVisibility(com.cfk.xiaov.R.id.llError, View.GONE).setViewVisibility(com.cfk.xiaov.R.id.cpbLoading, View.VISIBLE);
                        bivPic.showShadow(true);
                    } else if (sentStatus == Message.SentStatus.FAILED) {
                        helper.setViewVisibility(com.cfk.xiaov.R.id.llError, View.VISIBLE).setViewVisibility(com.cfk.xiaov.R.id.cpbLoading, View.GONE);
                        bivPic.showShadow(false);
                    } else if (sentStatus == Message.SentStatus.SENT) {
                        helper.setViewVisibility(com.cfk.xiaov.R.id.llError, View.GONE).setViewVisibility(com.cfk.xiaov.R.id.cpbLoading, View.GONE);
                        bivPic.showShadow(false);
                    }
                } else {
                    Message.ReceivedStatus receivedStatus = item.getReceivedStatus();
    ...
    private void setAvatar(LQRViewHolderForRecyclerView helper, Message item, int position) {
        ImageView ivAvatar = helper.getView(com.cfk.xiaov.R.id.ivAvatar);
        UserInfo userInfo = DBManager.getInstance().getUserInfo(item.getSenderUserId());
        if (userInfo != null) {
    ...
    private void setName(LQRViewHolderForRecyclerView helper, Message item, int position) {
        if (item.getConversationType() == Conversation.ConversationType.PRIVATE) {
            helper.setViewVisibility(com.cfk.xiaov.R.id.tvName, View.GONE);
        } else {
            helper.setViewVisibility(com.cfk.xiaov.R.id.tvName, View.GONE);
    ...
    private void setTime(LQRViewHolderForRecyclerView helper, Message item, int position) {
        ...
        if (position > 0) {
            ...
            if (msgTime - preMsgTime > (5 * 60 * 1000)) {
                helper.setViewVisibility(com.cfk.xiaov.R.id.tvTime, View.VISIBLE).setText(com.cfk.xiaov.R.id.tvTime, TimeUtils.getMsgFormatTime(msgTime));
            } else {
                helper.setViewVisibility(com.cfk.xiaov.R.id.tvTime, View.GONE);
            }
        } else {
            helper.setViewVisibility(com.cfk.xiaov.R.id.tvTime, View.VISIBLE).setText(com.cfk.xiaov.R.id.tvTime, TimeUtils.getMsgFormatTime(msgTime));
        }
