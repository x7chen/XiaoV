...
//        LogUtils.e(sb.toString());
    ...
    private void setAdapter() {
        if (mAdapter == null) {
            mAdapter = new LQRAdapterForRecyclerView<Geo2AddressResultObject.ReverseAddressResult.Poi>(mContext, mData, com.cfk.xiaov.R.layout.item_location_poi) {
                @Override
                public void convert(LQRViewHolderForRecyclerView helper, Geo2AddressResultObject.ReverseAddressResult.Poi item, int position) {
                    helper.setText(com.cfk.xiaov.R.id.tvTitle, item.title).setText(com.cfk.xiaov.R.id.tvDesc, item.address)
                            .setViewVisibility(com.cfk.xiaov.R.id.ivSelected, mSelectedPosi == position ? View.VISIBLE : View.GONE);
                }
            };
