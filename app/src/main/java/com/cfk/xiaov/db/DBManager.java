...
import com.cfk.xiaov.util.PinyinUtils;
...
public class DBManager {
            ...
            ApiRetrofit.getInstance().getGroupInfo(groupId)
                    ...
                    .subscribe(getGroupInfoResponse -> {
                        if (getGroupInfoResponse != null && getGroupInfoResponse.getCode() == 200) {
                            ...
                            if (groupInfo != null) {
                                String role = groupInfo.getCreatorId().equalsIgnoreCase(AccountCache.getAccount()) ? "0" : "1";
    ...
    public UserInfo getUserInfo(String userid) {
        ...
        if (groupMembers != null && groupMembers.size() > 0) {
            ...
            userInfo = new UserInfo(groupMember.getUserId(), groupMember.getName(), groupMember.getPortraitUri());
            return userInfo;
        }
...
//        deleteGroupMembers();
    ...
    public boolean isMe(String userid) {
        if (AccountCache.getAccount().equalsIgnoreCase(userid)) {
            return true;
        }
        return false;
    }
...
//        return DataSupport.findAll(Friend.class);
        return DataSupport.where("userid != ?", AccountCache.getAccount()).find(Friend.class);
    }
...
//                    }
                    ...
                    userInfo = new UserInfo(friend.getUserId(), name, portrait);
        ...
        if (groupMember != null) {
            if (TextUtils.isEmpty(groupMember.getPortraitUri())) {
                if (TextUtils.isEmpty(groupMember.getUserId())) {
                    ...
                    userInfo = new UserInfo(groupMember.getUserId(), groupMember.getName(), portrait);
